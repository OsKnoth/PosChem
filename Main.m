...
switch Bsp
  ...
  case 'Robertson'
    [Reak,Species,tBegin,tEnd,dt,dtMax]=Robertson();%Fehler?
  case 'Brusselator'
    [Reak,Species,tBegin,tEnd,dt,dtMax]=Brusselator();%Brusselator
  case 'Tilgner'
    [Reak,Species,tBegin,tEnd,dt]=Tilgner();%Tilgner
end
...
StMatrix = StochioMatrix(Reak,Species);
%Positivitätsabfrage des Reagktionssystems
% positive()

%Anfangswert y0
y=zeros(size(Species));
MolMass=zeros(size(Species));
for i=1:size(Species,1)
    y(Species(i).Pos)=Species(i).c;
    MolMass(Species(i).Pos)=Species(i).MolMass;
end

[P,D]=PDMatrix(tBegin,y);
MPD=P-D;
M=MHatMatrix(Reak,Species,tBegin,y);
...
%%
%ODE Solver
options = odeset('RelTol',1e-9,'AbsTol',1e-12,'Jacobian',@Jacobian);
% options = odeset('RelTol',1e-3,'AbsTol',1e-3);
[tt_tempF,yy_tempF] = ode23s(@odefun,[tBegin,tEnd],y,options);


%Eigene Solver
RKExp=RungeKutta('RK2a');
RK=RungeKutta('RK2a');
%RK=RungeKutta('RK2b');
%RKPat=RungeKutta('SSP1');
% RKPat=RungeKutta('meister3');
%RKPat=RungeKutta('meister2');
RKPat=RungeKutta('meister1');

yRK=zeros(size(y,1),4);
yRKExp=zeros(size(y,1),4);
yRKPat=zeros(size(y,1),4);
yRKPatPD=zeros(size(y,1),4);
errRK=zeros(4,1);
errRKExp=zeros(4,1);
errRKPat=zeros(4,1);
errRKPatPD=zeros(4,1);
err1=zeros(4,1);
err2=zeros(4,1);
err3=zeros(4,1);
h=zeros(4,1);
...
%p_1=loglog(h,errRKExp,'-o');hold on;
p_1=loglog(h,errRKPatPD,'-o');hold on;
